...
DDoS = ['DDoS-ACK_Fragmentation', 'DDoS-UDP_Flood', 'DDoS-SlowLoris', 'DDoS-ICMP_Flood',
        'DDoS-RSTFINFlood', 'DDoS-PSHACK_Flood', 'DDoS-HTTP_Flood', 'DDoS-UDP_Fragmentation',
        'DDoS-TCP_Flood', 'DDoS-SYN_Flood', 'DDoS-SynonymousIP_Flood', 'DDoS-ICMP_Fragmentation']
...
Web_based = ['SqlInjection', 'BrowserHijacking', 'CommandInjection', 'XSS', 'Backdoor_Malware', 'Uploading_Attack']
Mirai = ['Mirai-greeth_flood', 'Mirai-udpplain', 'Mirai-greip_flood']

# Classify attacks
df['label'].replace(DDoS, 'DDoS', inplace=True)
df['label'].replace(DoS, 'DoS', inplace=True)
df['label'].replace(Spoofing, 'Spoofing', inplace=True)
df['label'].replace(Brute_force, 'Brute_Force', inplace=True)
df['label'].replace(Recon, 'Recon', inplace=True)
df['label'].replace(Web_based, 'Web_based', inplace=True)
df['label'].replace(Mirai, 'Mirai', inplace=True)
...
X_train, X_test, Y_train, Y_test = train_test_split(
    X_scaled, Y, test_size=0.2, random_state=33, shuffle=True)
...
np.save('./results/history/CNN_Multiclass_classification_history.npy', history.history)
np.save('./results/history/CNN_Multiclass_classification_Fitting_time.npy', np.array([[start_fit, end_fit, fit_time]]))
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('./results/figures/CNN_Multiclass_classification_accuracy.eps', format='eps', dpi=1200)
plt.show()
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('./results/figures/CNN_Multiclass_classification_loss.eps', format='eps', dpi=1200)
plt.show()
...
np.save('./results/history/CNN_Multiclass_classification_Inference_time.npy', np.array([[start_time, end_time, inference_time]]))
...
labels = ['BenignTraffic', 'Brute_Force', 'DDoS', 'DoS', 'Mirai', 'Recon', 'Spoofing', 'Web_based']
plt.figure(figsize=(25,10))
sn.heatmap(pd.DataFrame(confMat), annot=True, fmt='g', xticklabels=labels, yticklabels=labels, cmap='Blues')
plt.ylabel('Actual Class')
plt.xlabel('Predicted Class')
plt.savefig('./results/figures/CNN_Multiclass_classification_confusion_matrix.eps', format='eps', dpi=1200)
plt.show()

# Classification report
class_report = classification_report(y_test, pred, target_names=labels, output_dict=True)
class_report_df = pd.DataFrame(class_report).transpose()
class_report_df.to_csv(r'./results/csvs/CNN_Multiclass_classification_report.csv')
...
# Save the model
model_save_path = os.path.join(output_dir, 'cnn_multiclass_model.keras')
